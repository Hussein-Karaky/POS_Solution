...
function DropDown(text, clsArr, autoComplete, options, selectorType) {
    ParentNode.call(this, selectorType);
    this.text = text;
    this.cssClasses = clsArr;
    this.autoComplete = autoComplete;
    this.options = options;
    this.onSelectionChanged = null;
};
DropDown.prototype = Object.create(ParentNode.prototype);
Object.defineProperty(DropDown.prototype, 'constructor', {
    enumerable: false,
    configurable: false,
    writable: false,
    value: DropDown
...
DropDown.prototype.create = function (domParentId) {
    ...
    this.cssClasses.forEach(cssClass => btn.classList.add(cssClass));
...
function DropDownList(text, clsArr, options) {
    DropDown.call(this, text, clsArr, "off", options, "select");
};
DropDownList.prototype = Object.create(DropDown.prototype);
Object.defineProperty(DropDownList.prototype, 'constructor', {
    enumerable: false,
    configurable: false,
    writable: false,
    value: DropDownList
...
DropDownList.prototype.create = function (domParentId) {
    ...
    this.cssClasses.forEach(cssClass => select.classList.add(cssClass));
...
DropDownList.prototype.data = function (domParentId) {
    let dom = this.getValueDOM();
    return {
        index: parseInt(dom.options[dom.selectedIndex].value), value: dom.options[dom.selectedIndex].text
    };
};
...
//Node.prototype.parse = function (json) { this.data = json; };
//Node.prototype.display = function (domParentId) { };

function CheckBox(label, value) {
    Node.call(this, "check");
    this.label = label;
    this.value = value;
};
CheckBox.prototype = Object.create(Node.prototype);
...
function MultiSelectList(options) {
    ParentNode.call(this, "ul");
    this.options = options !== null && options !== undefined ? options : [{ index: 0, value: "All" }];
};
MultiSelectList.prototype = Object.create(ParentNode.prototype);
Object.defineProperty(MultiSelectList.prototype, 'constructor', {
    enumerable: false,
    configurable: false,
    writable: false,
...
MultiSelectList.prototype.create = function (domParentId) {
    ...
    this.options.forEach(option => {
        ...
        let check = new CheckBox(option.value, option.index);
        main.add(check);
...
TimeLimit.prototype = Object.create(ParentNode.prototype);
Object.defineProperty(TimeLimit.prototype, 'constructor', {
    enumerable: false,
    configurable: false,
    writable: false,
    value: TimeLimit
...
TimeLimit.prototype.create = function (domParentId) {
    ...
    this.from = new DropDownList("", ["form-control"], dayHours());
    this.from.create(frontCard.id);
    let sep = document.createElement('span');
    sep.innerHTML = "&nbsp;to&nbsp;";
    frontCard.appendChild(sep);
    this.to = new DropDownList("", ["form-control"], dayHours());
    ...
    btnClose.onclick = function (e) {
        if (main.parent !== null && main.parent !== undefined) {
            $(main.getDOM()).animate({
                opacity: 0,
                marginLeft: '-200px'
            }, 'medium', 'linear', function () { main.parent.remove(main); });
            
        } else {
            $(main.getDOM()).animate({
                opacity: 0,
                marginLeft: '-200px'
            }, 'medium', 'linear', function () { main.delete(); });
        }
    };
    frontCard.appendChild(btnClose);
    this.extra.phoneCall = new CheckBox("Phone calls", true);
    ...
    this.extra.onlineSession = new CheckBox("Online only", true);
    ...
    this.extra.faceToFace = new CheckBox("Face to face", true);
    ...
    ["ml-2", "mb-1", "close"].forEach(c => btnFlipBack.classList.add(c));
    btnFlipBack.setAttribute("data-dismiss", "toast");
    btnFlipBack.setAttribute("aria-label", "Close");

    btnFlipBack.setAttribute("style", "top: 28%; position: relative; right: 92%;");
...
ScheduleDay.prototype = Object.create(ParentNode.prototype);
Object.defineProperty(ScheduleDay.prototype, 'constructor', {
    enumerable: false,
    configurable: false,
    writable: false,
    value: ScheduleDay
});
ScheduleDay.prototype.defaultStartTime = 8;
ScheduleDay.prototype.render = function (oldLength, length, limit) {
    limit.create(this.getDOM().id);
    if (length > 1) {
        this.children[oldLength - 1].setAfter(limit);
    } else {
        limit.from.select(ScheduleDay.prototype.defaultStartTime);
    }
};
ScheduleDay.prototype.add = function (limit) {
    if (limit !== null && limit !== undefined) {
        //let oldLength = this.children !== null ? this.children.length : 0;
...
ScheduleDay.prototype.remove = function (limit) {
...
ScheduleDay.prototype.calculateHeight = function () {
...
ScheduleDay.prototype.onChildAdded = function (children) {
    $(this.getDOM()).closest(".flip-card-inner").animate({ height: this.calculateHeight() + "px" }, 400);
};
ScheduleDay.prototype.onChildRemoved = function (children) {
    $(this.getDOM()).closest(".flip-card-inner").animate({ height: this.calculateHeight() + "px" }, 400);
};
ScheduleDay.prototype.create = function (domParentId) {
    ...
    this.extra.active = new CheckBox(this.day.value, false);
    ...
    this.extra.active.getValueDOM().onchange = function (e) {
        ...
        $($(main.getDOM()).closest(".flip-card-inner").find(".btn-edit")[0]).css("display", main.dat !== null && main.dat !== undefined && main.children.length > 0 ? "inline-block" : "none");
    };
    ...
    btnOK.onclick = function (e) {
        ...
        card.animate({ height: main.calculateHeight() + "px" }, 400);
        ...
        //if (main.children !== null && main.children !== undefined && main.children.length > 0) {
        //    main.extra.active.getDOM().getElementsByTagName("input")[0].dispatchEvent(new Event('change'));
        //}
    ...
    btnPlus.onclick = function (e) {
        ...
        //$(inCard).animate({ height: main.calculateHeight() + "px" }, 500);
    };
    ...
    collapser.setAttribute("style", "top: 28%; position: relative;");

    collapser.innerHTML = //"Copy";
        "<svg xmlns=\"http://www.w3.org/2000/svg\" xmlns:xlink=\"http://www.w3.org/1999/xlink\" x=\"0px\" y=\"0px\" viewBox=\"-55 -55 1000 1000\" style=\"enable-background:new -55 -55 1000 1000;\" xml:space=\"preserve\"><g><g><g><path style=\"fill:#007bff;\" d=\"M335.328,332.459V44.569c0-13.079-10.64-23.711-23.719-23.711H23.711	C10.632,20.858,0,31.482,0,44.569v287.89c0,13.079,10.624,23.719,23.711,23.719h287.898" +
    ...
    this.extra.selection = new MultiSelectList(filtered);
    this.extra.selection.create(card.id);

    frontCard.appendChild(this.getDisplay());
    return this;
};
ScheduleDay.prototype.getDisplay = function () {
    let main = this;
    div = document.createElement("div");
    div.classList.add(this.constructor.name.concat("-Display"));
    let btnEdit = document.createElement('button');
    btnEdit.type = "button";
    ["ml-2", "mb-1", "close", "btn", "btn-primary", "btn-edit"].forEach(c => btnEdit.classList.add(c));
    ...
    btnEdit.onclick = function (e) {
        main.editing = true;
        $(main.getDOM()).closest(".flip-card-inner").animate({ height: main.calculateHeight() + "px" }, 400);
        $(main.getDOM()).closest(".flip-card-inner").css("transform", "rotateY(180deg)");
    };
    $(btnEdit).css("display", main.dat !== null && main.dat !== undefined && main.dat.limits.length > 0 ? "inline-block" : "none");
    div.appendChild(btnEdit);

    let el = ParentNode.prototype.getDisplay.call(this);
    div.appendChild(el);
    return div;
};
ScheduleDay.prototype.clear = function (skip) {
    ...
    if (this.dat !== null && this.dat !== undefined && this.dat.limits.length > 0) {
        ...
        $(this.getDOM()).closest(".flip-card-inner").animate({ height: "5px"}, 400);
    }
    this.onEmpty();
};
ScheduleDay.prototype.data = function () {
    let data = {
        day: this.day,
...
ScheduleDay.prototype.parse = function (data) {
    if (data !== null) {
        Node.prototype.parse.call(this, data);
        ...
        //    $(main.getDOM()).closest(".flip-card-inner").animate({ height: 100 * main.children.length + "px" }, 400);
        //    $(main.getDOM()).closest(".flip-card-inner").css("transform", "rotateY(180deg)");
        //});

    }
};
ScheduleDay.prototype.onEmpty = function () {
    $(this.getDOM()).find(".TimeLimit").each(function () { $(this).detach().appendTo($(recycleBin)); });
};
ScheduleDay.prototype.onContentChanged = function () {
...
function Schedule() {
    ParentNode.call(this, "div");
    weekDays.forEach(day => this.add(new ScheduleDay(day)));
};
Schedule.prototype = Object.create(ParentNode.prototype);
Object.defineProperty(Schedule.prototype, 'constructor', {
    enumerable: false,
    configurable: false,
    writable: false,
    value: Schedule
});
Schedule.prototype.create = function (domParentId) {
    let div = document.createElement('div');
    div.classList.add(this.constructor.name);
    this.setSelectorIndex(document.getElementsByClassName(this.constructor.name).length);
    div.id = this.constructor.name.concat(this.selectorIndex);
...
Schedule.prototype.data = function () {
    let data = {
        days: []
    };
...
Schedule.prototype.parse = function (data) {
    if (data !== null) {
        let main = this;
        this.dat = data;
...
Schedule.prototype.fetch = function (onError) {
    $.ajax({
        type: "POST",
        url: "https://localhost:44317/tutor/getAvailSvc",
        data: null,
        success: function (data, text) { sched.parse(data); },// sign function (data, text)
        error: onError// sign function (request, status, error)
    });
};
Schedule.prototype.save = function (onSuccess, onError) {
    let data = this.data();
    $.ajax({
        type: "POST",
        url: "https://localhost:44317/tutor/saveAvailSvc",
