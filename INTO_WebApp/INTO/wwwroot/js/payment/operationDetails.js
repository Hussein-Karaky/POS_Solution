...
var Actions = document.querySelectorAll('td.action');
Actions.forEach(function (elment) {
    var CanRefund = elment.getAttribute("data-CanRefund");
    var CanVoid = elment.getAttribute("data-CanVoid");
    if (CanRefund == "true") {
        elment.firstElementChild.innerHTML = "Return";
    } else if (CanVoid == "true") {
        elment.firstElementChild.innerHTML = "Cancel";
    } else {
        elment.innerHTML = "";
    }
});
...
//let closeBtn='<button onclick="removeFilter.call()" type="button" class="close" aria-label="Close"><span aria-hidden="true">&times;</span></button>';
// let filtredDate=document.querySelector('.date-filter').innerHTML;
// filterTags.firstElementChild.innerHTML="<strong>date</strong> ="+filtredDate+closeBtn;
// filterTags.firstElementChild.setAttribute('data-value',todayFilter);

function OpenOprDetails(row) {
    $('.modal-dialog').load(window.location.origin.concat("/Payment/OprDetails/").concat(row.getData().OprId), function () {
        $('#myModal').modal({ show: true });
    });
}

let defaultOprId = parseInt(document.querySelector('#myModal').getAttribute('data-opr'));
if (defaultOprId != 0) OpenOprDetails(defaultOprId);
...
function getDate(d) {
    ...
    let todaystring = date.getFullYear() + '-0' + m + '-' + date.getDate();
    return todaystring;
}
